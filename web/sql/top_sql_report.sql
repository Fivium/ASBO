DECLARE
--
-- $Id: //Infrastructure/GitHub/Database/asbo/web/sql/top_sql_report.sql#1 $
--
-- 2012 - T Dale - quick top sql report, runs on oracle standard, with a snapper
--
  ...
  IS
    ...
    CURSOR c_is_enterprise_edition  
    IS 
      SELECT 
        'YES' enterprise_edition
      FROM  
        v$version
      WHERE
        lower(banner) like '%enterprise%';
 
    l_is_enterprise_edition c_is_enterprise_edition%ROWTYPE;
  BEGIN
    OPEN c_is_enterprise_edition;
      FETCH c_is_enterprise_edition INTO l_is_enterprise_edition;
      IF c_is_enterprise_edition%FOUND THEN
        l_return_val := TRUE;
      END IF;
    CLOSE c_is_enterprise_edition;
...
BEGIN
  ...
  OPEN c_top_sql FOR
    ...
    FROM (
      ...
      , RANK () OVER (ORDER BY '|| order_report_by ||')        sql_rank
      FROM 
        __hist_sqlstat__ sqlstat
      , __hist_sqltext__ sqltext
      WHERE 
