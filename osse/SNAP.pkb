...
--
-- $Id: //Infrastructure/GitHub/Database/avo/osse/SNAP.pkb#1 $
--
-- T Dale 2012
    ...
    FOR i IN 1..60 LOOP
      FOR i IN 1..60 LOOP
        ...
        ( 
          ...
          , s.machine  
          FROM 
               sys.active_session_details sd
          JOIN v$session                  s  ON s.sid = sd.session_id
        );
      ...
      USING 
        dbamgr.session_snaps ss ON (sh.sample_time=ss.sample_time AND sh.session_id=ss.session_id)  
      WHEN NOT MATCHED THEN
        INSERT (sh.sample_time,sh.session_type,sh.session_state,sh.wait_class,sh.event,sh.sql_id,sh.session_id,sh.user_id,sh.program,sh.module)
        VALUES (ss.sample_time,ss.session_type,ss.session_state,ss.wait_class,ss.event,ss.sql_id,ss.session_id,ss.user_id,ss.program,ss.module);
       
    END LOOP;
    --
    -- Remove old snaps
    --
    DELETE FROM DBAMGR.SESSION_SNAPS      WHERE sample_time < SYSDATE - 8/24;
    DELETE FROM DBAMGR.SESSION_SNAPS_HIST WHERE sample_time < SYSDATE - 2;  
    COMMIT;
